...
function request_name() {
    ...
    print "<p>In 1980, Romana Anastacio translated the classic children's novel, CHARLOTTE'S WEB, into Palauan.  Unfortunately, there are only a few copies
                remaining in the world.  We are trying to translate more novels so that make we can make them available as free eBooks and make them available for purchase as 
                print-bound books.  Note that tekinged.com does not, and never will, make any money from this nor from anything else.\n";
    echo "<p>Thanks for your interest in helping to create online Palauan language resources! 
          <p>The current task is to help translate the $book novel. 
          ";
...
function record_work() {
  ...
  $text = $_POST['translation'];
  $row = $_POST['row'];

  $text = utf8_encode($text); # fix any encoding
  $text = mysql_real_escape_string($text);
  $q = "submitted=now(),palauan='$text'";
  update_table($q,$row);
...
function add_instructions() {
  ...
  print "<b>Instructions:</b>
        <p>Do your best to translate the English paragraph from the novel into Palauan.  
        <ul>
...
function add_progress() {
  $total = my_get_count();
  $done = my_get_count("where !isnull(submitted)");
...
function next_task() {
  ...
  print "<table><tr>
          <td align='top'>Please translate this text: <br>$text</td>
          <td align='top'><textarea class='kerresel' rows='5' cols='50' name='translation' placeholder='Moluches er tiang'></textarea></td>
          </tr></table><br>\n";
