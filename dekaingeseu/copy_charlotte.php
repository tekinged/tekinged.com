...
function add_instructions() {
  print "<p>In 1980, Romana Anastacio translated the classic children's novel, CHARLOTTE'S WEB, into Palauan.  Unfortunately, there are only a few copies
                remaining in the world.  We are trying to restore and digitize so we can re-release it as a free eBook and make it available for purchase as a 
                print-bound book.  Note that tekinged.com does not, and never will, make any money from this nor from anything else.\n";
  print "<p><ul><li>Copy the page on the left into the textbox on the right.
                ...
                <li>Read the story and check progress so far <a href=/books/translation.php?book=translate_charlotte>here</a>.
         </ul>";
}

function copy_charlotte_record_task($result) {
  $text = $_POST['newtext'];
...
function copy_charlotte_make_task($db_row) {
  add_instructions();
  $page = $db_row['page'];
  $id   = $db_row['id'];
  ...
  $q = "select id,pal from dekaingeseu_charlotte where $where";
  ...
  $img_path = "/books/charlotte.images/page-"; 
...
$config['q_find'] = "select id,page from dekaingeseu_charlotte where $where";
$config['order'] = 'id';
$config['get_count'] = "where $where";
$config['table'] = 'dekaingeseu_charlotte';
$config['make_task'] = "copy_charlotte_make_task"; # function pointer
$config['record_task'] = "copy_charlotte_record_task"; # function pointer
