...
# was there a constructed URL to force a lookup?
if (isset($_GET['p'])) {
  Debug("Force lookup is on");
  $force_lookup = true;
  $target = $_GET['p'];
  $direction = 'pe'; 
}
...
if( $force_lookup && (strlen($target)>0 || $specific_target != NULL) ) {
    ...
    if ($specific_target != Null) {
      $query = "select id,stem from all_words3 where id=$specific_target";
      $entries = get_words($query);
    } else {
    ...
    if (!$found) {
        ...
        $examples_links = "";
        foreach( array('sentences','proverbs') as $table ) {
          $mywhere = "where $example_lang regexp '" . '[[:<:]]' . $target . '[[:>:]]' . "'";
...
function add_aside($empty=False) {
  ...
  if ($empty == False) {
    ...
    echo "
          ...
          <br>
          <br>
          Latest additions: <a href=/3000.php>3000 Top Words</a> and <a href=/quiz/q_synonyms.php>Synonym Quiz</a>!
          </div><!-- tab -->
      ";
